<template>
  <div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      ...
      <div v-else-if="solutions.length === 0" class="p-8 text-center text-slate-400 text-sm">
        Aucune solution.
      </div>
        ...
        <table class="w-full text-sm min-w-[640px]">
          <thead class="bg-slate-50 border-b border-slate-100">
            <tr>
              ...
              <th
                class="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider hidden md:table-cell"
              >
                Catégorie
              </th>
              <th
                class="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider hidden lg:table-cell"
              >
                Prix
              </th>
              <th
            ...
            <tr v-for="sol in solutions" :key="sol.id" class="hover:bg-slate-50 transition-colors">
              ...
              <td class="px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <button
                    @click="openForm(sol.id)"
                    class="p-1.5 rounded-lg hover:bg-slate-100 text-slate-500 hover:text-green-600 transition-colors"
                    title="Modifier"
                  >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          <div
            class="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white"
          >
            <h2 class="font-bold text-slate-900">
              {{ form.id ? 'Modifier la solution' : 'Nouvelle solution' }}
            </h2>
            <button
          ...
          <div class="px-6 py-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="col-span-2">
                ...
                <input
                  ...
                  class="field"
                  placeholder="La solution complète pour..."
                />
              </div>
              <div>
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1.5"
              ...
              <div class="col-span-2">
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1.5"
                  >Présentation PDF</label
                >
                <div class="flex gap-3">
                  <input
                    v-model="form.brochure_url"
                    class="field flex-1"
                    placeholder="https://exemple.com/presentation.pdf"
                  />
                  <label
                    class="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium cursor-pointer transition-all flex-shrink-0"
                  >
                    Uploader PDF
                    <input
                      type="file"
                ...
                <p class="text-xs text-slate-400 mt-1">
                  Uploader un PDF de présentation ou coller l’URL directe.
                </p>
                <p v-if="form.brochure_url" class="text-xs mt-2">
                  <a
                    :href="form.brochure_url"
                    target="_blank"
                    class="text-green-600 hover:underline"
                    >Voir le PDF actuel</a
                  >
                </p>
              </div>
              <div class="col-span-2">
                <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-1.5"
                  >Lien vidéo de démo</label
                >
                <input v-model="form.demo_url" class="field" placeholder="https://youtu.be/..." />
                <p class="text-xs text-slate-400 mt-1">
            ...
            <div>
              ...
              <div class="flex gap-3">
                <input
                  ...
                  class="field flex-1"
                  placeholder="URL ou chemin après upload"
                />
                <label
                  class="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium cursor-pointer transition-all flex-shrink-0"
                >
              ...
              <img
                ...
                :src="form.hero_image"
                class="mt-3 h-24 w-auto rounded-xl object-cover border border-slate-200"
              />
            </div>
            ...
            <div>
              <label class="block text-xs font-bold text-slate-600 uppercase tracking-wide mb-3"
                >Statistiques (3 max)</label
              >
              <div class="grid grid-cols-3 gap-3">
                <div v-for="i in [1, 2, 3]" :key="i" class="space-y-2">
                  <input
                    v-model="form[`stat${i}_value`]"
                    class="field text-sm"
                    :placeholder="`Valeur ${i}`"
                  />
                  <input
                    v-model="form[`stat${i}_label`]"
                    class="field text-sm"
                    :placeholder="`Label ${i}`"
                  />
                </div>
            ...
            <!-- Modules removed from admin form (managed elsewhere) -->
            ...
            <div>
              ...
              <div class="space-y-2">
                <div v-for="(_, i) in form.advantages" :key="i" class="flex gap-2">
                  <input
                    v-model="form.advantages[i]"
                    class="field flex-1 text-sm"
                    placeholder="Avantage..."
                  />
                  <button
                    @click="form.advantages.splice(i, 1)"
            ...
            <div>
              ...
              <input
                v-model="tagsInput"
                class="field"
                placeholder="Facturation, Pharmacie, Statistiques"
              />
            </div>
            ...
            <div class="flex gap-4">
              ...
              <div>
                <label class="text-xs font-bold text-slate-600 uppercase tracking-wide mr-2"
                  >Ordre</label
                >
                <input
                  ...
                  type="number"
                  min="0"
                  class="w-16 px-2 py-1 rounded-lg border border-slate-200 text-sm text-center"
                />
              </div>
            </div>
          ...
          <div
            class="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 sticky bottom-0 bg-white"
          >
            <button
              @click="showForm = false"
              class="px-5 py-2.5 rounded-xl border border-slate-200 text-sm font-medium hover:bg-slate-50 transition-colors"
            >
              Annuler
            </button>
            <button
              @click="submitForm"
              :disabled="submitting"
              class="px-5 py-2.5 rounded-xl bg-green-600 hover:bg-green-500 disabled:opacity-60 text-white text-sm font-bold transition-all"
            >
    ...
    <Teleport to="body">
      ...
      >
        <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center">
          <div
            class="w-14 h-14 rounded-full bg-red-100 flex items-center justify-center mx-auto mb-10"
          >
          ...
          <p class="text-sm text-slate-500 mb-10">
            Cette action est irréversible. Les modules, avantages et tags associés seront également
            supprimés.
          </p>
          <div class="flex gap-3 justify-center">
            <button
              @click="deleteTarget = null"
              class="px-5 py-2.5 rounded-xl border border-slate-200 text-sm font-medium hover:bg-slate-50"
            >
              Annuler
            </button>
            <button
              @click="deleteSolution"
              class="px-5 py-2.5 rounded-xl bg-red-600 hover:bg-red-500 text-white text-sm font-bold"
            >
              Supprimer
            </button>
          </div>
...
async function uploadPdf(e) {
  ...
  const fd = new FormData()
  fd.append('file', file)
  fd.append('type', 'solution_pdf')
  try {
    const { data } = await api.post('/admin/upload', fd, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
    form.brochure_url = data.url
  } catch {
    formError.value = "Erreur lors de l'upload du PDF."
  }
}
