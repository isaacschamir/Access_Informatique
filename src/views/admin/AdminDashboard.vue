  ...
  <div>
    <div class="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="text-slate-500 mt-1">
          Bonjour {{ adminStore.admin?.name }} — voici l'état du site.
        </p>
      </div>
      <button
        v-if="adminStore.admin?.role === 'editor'"
        @click="handleLogout"
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
      ...
      <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div v-else-if="lastContacts.length === 0" class="p-8 text-center text-slate-400 text-sm">
          Aucun message reçu.
        </div>
        <div v-else>
          <div
            v-for="c in lastContacts"
            :key="c.id"
            class="flex items-start gap-4 px-6 py-4 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors"
          >
            <div
              class="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 font-bold text-sm flex-shrink-0"
            >
      ...
      <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div v-if="loading" class="p-8 text-center text-slate-400 text-sm">Chargement...</div>
        <div
          v-else-if="lastInscriptions.length === 0"
        ...
        <div v-else>
          <div
            v-for="ins in lastInscriptions"
            :key="ins.id"
            class="flex items-start gap-4 px-6 py-4 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors"
          >
            <div
              class="w-9 h-9 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-bold text-sm flex-shrink-0"
            >
...
const stats = computed(() => [
  {
    ...
    value: counters.value.solutions ?? '—',
    new: 0,
    icon: ICON_SOL,
    bg: 'bg-blue-50',
    color: 'text-blue-600',
  },
  {
    ...
    value: counters.value.formations ?? '—',
    new: 0,
    icon: ICON_FORM,
    bg: 'bg-purple-50',
  ...
  },
  {
    ...
    value: counters.value.contacts_total ?? '—',
    new: counters.value.contacts_new ?? 0,
    icon: ICON_MSG,
    bg: 'bg-amber-50',
  ...
  },
  {
    ...
    value: counters.value.inscriptions_total ?? '—',
    new: counters.value.inscriptions_new ?? 0,
    icon: ICON_USR,
    bg: 'bg-green-50',
