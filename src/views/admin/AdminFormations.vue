<template>
  <div>
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      ...
      <div v-else-if="formations.length === 0" class="p-8 text-center text-slate-400 text-sm">
        Aucune formation.
      </div>
        ...
        <table class="w-full text-sm min-w-[640px]">
          <thead class="bg-slate-50 border-b border-slate-100">
            <tr>
              ...
              <th
                class="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase hidden md:table-cell"
              >
                Catégorie
              </th>
              <th
                class="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase hidden lg:table-cell"
              >
                Prix
              </th>
              <th class="px-6 py-3 text-center text-xs font-bold text-slate-500 uppercase">
                Actif
              </th>
              <th class="px-6 py-3 text-right text-xs font-bold text-slate-500 uppercase">
                Actions
              </th>
            </tr>
          </thead>
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          >
            <h2 class="font-bold text-slate-900">
              {{ form.id ? 'Modifier' : 'Nouvelle formation' }}
            </h2>
            <button
          ...
          <div class="px-6 py-6 space-y-5">
            <div class="grid grid-cols-2 gap-4">
              <div class="col-span-2">
                <label class="flabel">Titre *</label><input v-model="form.title" class="field" />
              </div>
              <div>
                <label class="flabel">Slug *</label
                ><input v-model="form.slug" class="field" placeholder="dev-web-fullstack" />
              </div>
              <div>
                <label class="flabel">Catégorie *</label
                ><input v-model="form.category" class="field" placeholder="Développement" />
              </div>
              <div>
                <label class="flabel">Durée</label
                ><input v-model="form.duration" class="field" placeholder="3 mois" />
              </div>
              <div>
                <label class="flabel">Niveau</label
                ><input v-model="form.level" class="field" placeholder="Débutant" />
              </div>
              <div>
                <label class="flabel">Prix *</label
                ><input v-model="form.price" class="field" placeholder="150 000 FCFA" />
              </div>
              <div class="col-span-2">
            ...
            <div>
              ...
              <div class="flex gap-3">
                ...
                <label
                  class="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium cursor-pointer flex-shrink-0"
                >
                  Uploader
              ...
              <img
                v-if="form.image_url"
                :src="form.image_url"
                class="mt-3 h-20 w-auto rounded-xl object-cover border border-slate-200"
              />
            </div>
            ...
            <div>
              <div class="flex items-center justify-between mb-2">
                <label class="flabel !mb-0">Modules</label
                ><button
                  @click="addModule"
            ...
            <div>
              <div class="flex items-center justify-between mb-2">
                <label class="flabel !mb-0">Bénéfices</label
                ><button
            ...
            <div>
              <div class="flex items-center justify-between mb-2">
                <label class="flabel !mb-0">Résultats attendus</label
                ><button
                  @click="form.outcomes.push('')"
                  class="text-xs text-green-600 font-semibold hover:underline"
                >
                  + Ajouter
                </button>
              ...
              <div class="space-y-2">
                <div v-for="(__, i) in form.outcomes" :key="i" class="flex gap-2">
                  <input
                    v-model="form.outcomes[i]"
                    class="field flex-1 text-sm"
                    placeholder="Compétence acquise..."
                  />
                  <button
                    @click="form.outcomes.splice(i, 1)"
                    class="p-2 text-red-400 hover:text-red-600"
                  >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
          ...
          >
            ...
            <button
              @click="submitForm"
              :disabled="submitting"
              class="px-5 py-2.5 rounded-xl bg-green-600 hover:bg-green-500 disabled:opacity-60 text-white text-sm font-bold"
            >
...
async function openForm(id = null) {
  ...
  if (id) {
    ...
    form.modules = (data.modules ?? []).map((m) => ({
      title: m.title,
...
function addModule() {
  form.modules.push({ title: '', description: '', duration: '' })
}
...
async function uploadImage(e) {
  ...
  try {
    ...
    formError.value = "Erreur lors de l'upload."
  }
}
...
async function deleteFormation() {
  ...
  try {
    await api.delete(`/admin/formations?id=${deleteTarget.value.id}`)
...
.field {
  @apply w-full px-3 py-2 rounded-xl border border-slate-200 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all;
}
.flabel {
