 ...
 * ---------------------------------------------------------------
 * Upload de fichiers depuis le dashboard admin.
 *
 * Form-data attendu :
 *   image|file = fichier (jpg, jpeg, png, webp, gif, pdf — max 5 Mo)
 *   type       = "solution" | "partner" | "formation" | "solution_pdf" | "partner_pdf" | "formation_pdf"
 *                (optionnel, défaut: "solution")
 *
 * Réponse succès :
 *   { "success": true, "url": "http://.../uploads/solutions/abc123.pdf" }
 *
 * Sécurité :
...
if (!$file || $file['error'] === UPLOAD_ERR_NO_FILE) {
    error_response('Aucun fichier reçu. Champ attendu : "image" ou "file".', 422);
}

if ($file['error'] !== UPLOAD_ERR_OK) {
    error_response('Erreur lors de l\'envoi du fichier (code : ' . $file['error'] . ').', 422);
}
...
if (!isset($allowed_mimes[$mime_type])) {
    error_response('Type de fichier non autorisé. Formats acceptés : JPG, PNG, WEBP, GIF, PDF.', 422);
}
...
$allowed_types  = ['solution', 'partner', 'formation', 'solution_pdf', 'partner_pdf', 'formation_pdf'];
if (!in_array($type, $allowed_types, true)) {
    $type = 'solution';
}
$sub_folder = preg_replace('/_pdf$/', '', $type) . 's'; // solution_pdf → solutions
...
$filename  = bin2hex(random_bytes(16)) . '.' . $extension; // ex: a3f9c7...ab12.pdf
